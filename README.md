...
부산지역의 과거 119 신고접수 데이터와 전일 오후 6시에서 오전 9시 데이터를 이용하여 오전 9시에서 오후 6시 시간별 신고접수량을 예측하는 대회.
...
### 대회소개:
119종합상황실 2교대근무 주간(9시 to 18시, 9시간) 야간(18시 to 9시, 15시간) 주어진 과거 및 전일 야간근무 시간내의 119신고접수 데이터를 이용하여 오전 9시에서 오후 6시(주간근무시간) 9시간동안의 신고접수량을 예측, 다시말해 시간별 신고 접수량을 예측하는 Regression 문제입니다.
...
- 인풋데이터를 보면 6시간 전을 포함하여 기준일로 전날의 마지막 6시간을 맞물려 데이터를 가지고 있습니다. 추후 접수량계산 시 해당일을 맞추는 번거로운작업을 피하기 위해 접수일시를 6시간을 이동하여 접수건과 접수일이 일치하도록 전처리를 진행했습니다.
...
어느정도 학습이 되는 것으로 확인하였으나,

<p align='center'><img src="assets/fig09.png" width="640"></p>

학습된 결과를 시각화 해본 결과 일관된 주기성은 예측 가능해보이나, 이상치에 대한 예측이 안되는 것으로 확인했습니다. 해당 날이 공휴일인지 아닌지에 따른 값들이 예측하는데 도움이 될 수도 있다는 판단도 있었으나, 이상치에 대한 진폭 큰 데이터가 해마다 일정한 공휴일에 해당되지 않다는 판단도 있었습니다. 
...
xgboost 모델에 넣기위해서 시간대별 피쳐들을 생성하고 그 날이 주중인지 주말인지에 대한 피쳐를 추가했습니다.
...
이후 public 리더보드의 순위를 위해 임의 하이퍼파라미터 튜닝을 추가 진행했습니다.
...
그리드서치통해 얻은 기본 하이퍼파라미터에 Early Stopping을 적용하여 학습한 서브미션이 가장 좋은 final 점수를 얻을수 있었으며, 여러차례 public에서 순위에 오르기 위한 임의로 시도한 하이퍼파라미터 튜닝들은 public 리더보드에서 과적합을 야기할뿐 final 리더보드 점수를 낮추기만 했습니다.

- Best Scoring: Public MAPE: 15.7228, Final MAPE: 15.6377

- Submmision: Public MAPE: 15.6143, Final MAPE: 15.8617

최종제출은 public에서 가장 최적 점수를 얻은 submission 을 제출했지만, final 리더보드에서 오히려 더 적합하지 않는 MAPE 가 나왔습니다.

우승자는 'optuna'를 사용하여 피쳐엔지니어링을 진행한점이 가장 큰 요인으로 보였고,EDA와 피쳐엔지니어링에 좀더 다양한 시도가 이루워지지 않은 한계점에서 아쉬움이 남았습니다.
